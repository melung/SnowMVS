...
def homo_warping(src_fea, src_proj, ref_proj, depth_values, vis_ETA=False, fn=None):
    ...
    # print("Depth",depth_values.shape)
    # print("Hr", Hr)
    # print("Hs", Hs)
    # print("Wr", Wr)
    # print("Ws", Ws)
...
def init_range(cur_depth, ndepths, device, dtype, H, W):
    ...
    #print('init interval:',new_interval)
    ...
    #print("init",depth_range_samples[0,1,0,0]-depth_range_samples[0,0,0,0])
...
def init_inverse_range(cur_depth, ndepths, device, dtype, H, W):
    ...
    #print("init",(1/(inverse_depth_min - inverse_depth_max))/7)
    #print("init",1./inverse_depth_hypo[0,-2,0,0]-1./inverse_depth_hypo[0,-1,0,0])
...
def schedule_inverse_range(inverse_min_depth, inverse_max_depth, normal_plane, ndepths, H, W):
    ...
    #print("stage",(1/((inverse_min_depth - inverse_max_depth)))/ (ndepths - 1))
    # print("stage",(1./inverse_depth_hypo[0,-2,:,:]-1./inverse_depth_hypo[0,-1,:,:]).mean())
    # print("stage max",(1./inverse_depth_hypo[0,-2,:,:]-1./inverse_depth_hypo[0,-1,:,:]).max())
    # print("stage min",(1./inverse_depth_hypo[0,-2,:,:]-1./inverse_depth_hypo[0,-1,:,:]).min())
...
def schedule_range(min_depth, max_depth, normal_plane, ndepths, H, W, sum_weight):
    ...
    #print("stage interval", new_interval)
    ...
    #print(new_interval[:,0,0])
    #print("stage",depth_range_samples[0,1,0,0]-depth_range_samples[0,0,0,0])
...
def NA_DCN(in_channels, kernel_size=3, stride=1, dilation=1, bias=True, group_channel=8, gn=False):
    if gn:
        return nn.Sequential(
            ...
            #torchvision.ops.DeformConv2d(in_channels, in_channels, kernel_size=kernel_size, stride=stride,  bias=bias),
...
class FPN4(nn.Module):
    ...
    def __init__(self, base_channels, gn=False, dcn=False):
        ...
        if self.dcn:
            ...
            self.dcn1 = DeformConv2dPack(base_channels * 8, base_channels * 8, kernel_size=3,stride=1,padding=1,deform_groups=1)
            self.dcn2 = DeformConv2dPack(base_channels * 4, base_channels * 4, kernel_size=3,stride=1,padding=1,deform_groups=1)
            self.dcn3 = DeformConv2dPack(base_channels * 2, base_channels * 2, kernel_size=3,stride=1,padding=1,deform_groups=1)
            self.dcn4 = DeformConv2dPack(base_channels * 1, base_channels * 1, kernel_size=3,stride=1,padding=1,deform_groups=1)
...
class NormalNet(nn.Module):
    ...
    def forward(self, prob_volume):
        #concat = F.cat((feature, depth_init), dim=1)
...
class ConfidenceNet(nn.Module):
    def __init__(self, input_depths=8, input_channel = 8,conv_name='ConvBnReLU3D'):
        super(ConfidenceNet, self).__init__()
        module = importlib.import_module("models.mvs4net_utils")
        # stride_conv_name = 'ConvBnReLU3D'
        # self.conv0 = getattr(module, stride_conv_name)(input_depths, input_depths, kernel_size=(3,3,3), pad=(1,1,1))
        # self.conv1 = getattr(module, stride_conv_name)(input_depths, input_depths, kernel_size=(3,3,3), pad=(1,1,1))
        # self.prob = nn.Conv3d(input_depths, 1, 1, stride=1, padding=0)
 
        # self.confidence1 = nn.Conv2d(input_channel, input_channel *2, 3, stride=1, padding=1)
        # self.confidence2 = nn.Conv2d(input_channel *2, input_channel *2, 1, stride=1, padding=0)
        # self.confidence3 = nn.Conv2d(input_channel *2, 1, 1, stride=1,padding=0, bias=False)


    def forward(self, x):
        # #print("input con1", x.size())
        # x = self.conv1(self.conv0(x))
        # #print("input con2", x.size())
        # x = self.prob(x)
        # #print("input con3", x.size())

        # x = self.confidence2(self.confidence1(x.squeeze(1)))
        # #print("input con4", x.size())

        # x = F.sigmoid(self.confidence3(x))
        # #print("input con5", x.size())
...
#         self.ndepth = in_channels
#         self.conv0 = nn.Sequential(
#                 Conv2d(1, 4, 5, padding=2),
#                 Conv2d(4, 4, 5, padding=2),
#                 Conv2d(4, 4, 3, padding=1),
#             )
#         self.res = ConvBnSigmoid(4, 1) 

#     def forward(self, weight):
#         d_weight =self.res(self.conv0(weight))
#         return d_weight
...
class stagenet(nn.Module):
    ...
    def forward(self, features, rotation, normal_plane, normalnet, confidencenet,proj_matrices, depth_hypo, regnet, stage_idx, group_cor=False, group_cor_dim=8, split_itv=1, fn=None):
        ...
        for src_idx, (src_fea, src_proj) in enumerate(zip(src_features, src_projs)):
            ...
            # elif stage_idx == 1:
            #     src_weight = F.avg_pool2d(F.relu((normal_plane*norv_R).sum(dim = 1)), kernel_size= 5, stride = 1, padding = 2, count_include_pad=False) + 0.01
            # elif stage_idx == 2:
            #     src_weight = F.avg_pool2d(F.relu((normal_plane*norv_R).sum(dim = 1)), kernel_size= 11, stride = 1, padding = 5, count_include_pad=False) + 0.01                
            # else:
            #     src_weight = F.avg_pool2d(F.relu((normal_plane*norv_R).sum(dim = 1)), kernel_size= 17, stride = 1, padding = 8, count_include_pad=False) + 0.01
            if stage_idx == 0:
                src_weight = F.relu((normal_plane*norv_R).sum(dim = 1))
            elif stage_idx == 1:
                src_weight = F.avg_pool2d(F.relu((normal_plane*norv_R).sum(dim = 1)), kernel_size= 3, stride = 1, padding = 1, count_include_pad=False) + 0.01
            elif stage_idx == 2:
                src_weight = F.avg_pool2d(F.relu((normal_plane*norv_R).sum(dim = 1)), kernel_size= 3, stride = 1, padding = 1, count_include_pad=False) + 0.01                
            else:
                src_weight = F.avg_pool2d(F.relu((normal_plane*norv_R).sum(dim = 1)), kernel_size= 3, stride = 1, padding = 1, count_include_pad=False) + 0.01
        
            sim.append(src_weight)
            #src_weight = F.relu((normal_plane*norv_R).sum(dim = 1)) + 0.01

            src_weight = src_weight[:,None,None,:,:]
            src_weight = src_weight.repeat(1,group_cor_dim,D,1,1)# B C D H W
        ...
        attn_weight, norm = regnet(cor_feats)  # B D H W
        ...
        #print(attn_max_indices)
        #print(attn_max_indices.size())
        
        # if stage_idx == 3:
        #     depth = depth_regression(attn_weight, depth_values=depth_hypo)
        # else:
        ...
        with torch.no_grad():
            ...
            #depth_clone = depth.unsqueeze(1).detach().clone()
            # est_normal_plane = normalnet(depth_clone, ax, ay)
            # est_normal_plane = est_normal_plane[0]
        
        ret_dict = {"depth": depth,"photometric_confidence": photometric_confidence, "hypo_depth": depth_hypo, "attn_weight": attn_weight, "normal_plane": est_normal_plane,"ax":ax,"ay":ay,"sum_weight":sum_weight[:,0,0,:,:],"weight":torch.unsqueeze(torch.stack(sim),0)}
        
        #print("stage: ",stage_idx," depth: ",depth_hypo[0,:,10,10])
        
        #ref_weight
        #print("ref",ref_weight.size())
        #print(ref_weight.min(),ref_weight.max())
        if self.inverse_depth:
            last_depth_itv = 1./depth_hypo[:,2,:,:] - 1./depth_hypo[:,1,:,:]
            #print(last_depth_itv)
            #print("lat",last_depth_itv.size())
...
def sinkhorn(gt_depth, hypo_depth, attn_weight, mask, iters, eps=1, continuous=False):
    ...
    # loss = (T_map * D_map).reshape(B, H*W,-1)[mask.reshape(B, H*W, 1)].sum(-1)
    # loss = (loss * w).mean()
