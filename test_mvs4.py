...
def save_scene_depth(testlist):
    ...
    with torch.no_grad():
        for batch_idx, sample in enumerate(TestImgLoader):
            ...
            imgs = sample["imgs"]
            #ref_mask = sample["mask"][0,0,:,:]
            #print(np.shape(ref_mask))
            #print("aa",np.shape(imgs))
            ...
            for filename, cam, img, depth_est, normal_est, photometric_confidence, weight in zip(filenames, cams, imgs, \
                                                            outputs["depth"], outputs["normal_plane"], outputs["photometric_confidence"],outputs["weight"]):
                #imgs = img.numpy()
                imgs = img.copy()
                print(np.shape(imgs))
                img = img[0]  #ref view
                ...
                save_pfm(normal_filename, normal_est.transpose((1,2,0)))
                cv2.imwrite(normal_filename[:-3]+"jpg", normal_est.transpose((1,2,0)))
                ...
                #photometric_confidence[ref_mask==0] = 0
                ...
                for i in range(weight.shape[0]):
                    ...
                    plt.imsave(weight_img_filename, weight[i,0,:,:], cmap="gray", vmin=0, vmax=1)
                    plt.imsave(weight_n_img_filename, weight[i,0,:,:]/sw[0,:,:], cmap="gray", vmin=0, vmax=0.5)
