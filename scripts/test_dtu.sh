#!/usr/bin/env bash
DTU_TESTPATH="/hdd1/lhs/dev/code/cascade-stereo/Change/cascade-stereo/CasMVSNet/data/DTU/dtu_test_all"
...
python test_mvs4.py --dataset=general_eval4 --batch_size=1 --testpath=$DTU_TESTPATH  --testlist=$DTU_TESTLIST --loadckpt $DTU_CKPT_FILE --interval_scale 1.06 --outdir $DTU_OUT_DIR\
             --inverse_depth --thres_view 3 --conf 0.3 --group_cor --attn_temp 2 $PY_ARGS | tee -a $DTU_LOG_DIR/log_test.txt
fi
#inverse!!!!!!